...
BEGIN {
    ...
    if ( !defined $ARGV[0] or $ARGV[0] eq '-c' or $ARGV[0] eq 'check') {
        my @modules = qw(JSON Carp File::Spec::Functions File::Basename 
            File::Find List::MoreUtils Expect Getopt::Long);
        my $count = @modules;
...
sub Main {
    ...
    my $help_msg = "Usage: dm <option> \n" .
                   # "add      add dotfile \n" .
                   # "check      check the dependences \n" .
                   # "init       \n" .
                   "--clear      clear the stored files \n" .
                   "--deploy     deploy in new env \n" .
                   "--edit       edit config file \n" .
                   "--help       show help message \n" .
                   "--link       link all dotfiles \n" .
                   "--push       push to remote \n" .
                   "--update     reload the config message \n" .
                   "--remove     remove the dir does not in you config \n" .
                   "--show       show all dotfile \n" .
                   "--sync       pull the repository \n";
        
    die $help_msg unless defined $ARGV[0];
    ...
    if ($is_help)      {print $help_msg;} 
    elsif ($is_clear)  {$self->clear();}
    elsif ($is_deploy) {$self->deploy();}
    elsif ($is_edit)   {$self->edit_config()}
    elsif ($is_link)   {$self->link_all();}
    elsif ($is_push)   {$self->push();}
    elsif ($is_update) {$self->update();}
    elsif ($is_remove) {$self->remove();}
    elsif ($is_show)   {$self->ls_app("output");}
    elsif ($is_sync)   {$self->sync();}
}
...
sub sync {
    ...
    if ( $git_installed == 0 ) {
        ...
        $expect->spawn( 'git', ( 'pull', "$remote_name", "master" ) )
            or carp "Something Wrong: $!";
        $expect->expect( 10, [qr/Username/] );
        $expect->send("$username\n");
        $expect->expect( 10, [qr/Password/] );
